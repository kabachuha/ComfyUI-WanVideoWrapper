...
def find_patch_max_indices(tensor, st, sx, sy, t, x, y):  # tensor: (B,N)
    ...
    h_patches = y // sy
    w_patches = x // sx
    tensor = tensor[:, :t_patches * st, :h_patches * sy, :w_patches * sx]
    tensor_reshaped = tensor.view(b, t_patches, st, h_patches, sy, w_patches, sx)
    tensor_reshaped = tensor_reshaped.permute(0, 1, 3, 5, 2, 4, 6).contiguous()
    tensor_reshaped = tensor_reshaped.view(b, t_patches, h_patches, w_patches, st * sy * sx)
...
def prune_and_recover_tokens(metric: torch.Tensor,
    ...
    if num_prune <= 0:  # 如果r<0, 什么也不做
        return do_nothing_s, do_nothing_s
    ...
    with torch.no_grad():
        ...
        rand_idx = find_patch_max_indices(dst_score, st, sx, sy, t, w, h)  # [B,hsy,wsx,1]
        # Align CFG
        ...
        # rand_idx = rand_idx.unsqueeze(0).expand(B,-1,-1,-1)
        ##############################################################
        idx_buffer_view = torch.zeros(B, hst, hsy, wsx, st * sy * sx, device=metric.device, dtype=torch.int64)
        ...
        idx_buffer_view = idx_buffer_view.view(B, hst, hsy, wsx, st, sy, sx).permute(0, 1, 4, 2, 5, 3, 6).reshape(B, hst * st, hsy * sy, wsx * sx)
        if (hst * st) < t or (hsy * sy) < h or (wsx * sx) < w:
            idx_buffer = torch.zeros(B, t, h, w, device=metric.device, dtype=torch.int64)
            idx_buffer[:, :(hst * st), :(hsy * sy), :(wsx * sx)] = idx_buffer_view  # (B,h,w)
        else:
...
def select_sito_method(x, grid_sizes): #, timestep):
    
    args = dict(prune_ratio = 0.5,
        max_downsample_ratio = 1,
        st = 1, sx = 2, sy = 2,
        noise_alpha= 0.1,
        sim_beta = 1)
    
    current_timestep = 6934 # unaffected
    original_t, original_h, original_w = tuple(grid_sizes[0].cpu())  # 64,64
    original_tokens = original_t * original_h * original_w
    downsaple_ratio = int(math.ceil(math.sqrt(original_tokens // x.shape[1])))

    if (downsaple_ratio <= args["max_downsample_ratio"]):
        t = int(math.ceil(original_t / downsaple_ratio))  
        ...
        p, r = prune_and_recover_tokens(x, num_prune, t=t, w=w, h=h, st=int(args['st']), sx=int(args['sx']), sy=int(args['sy']),noise_alpha=args['noise_alpha'],sim_beta=args['sim_beta'],current_timestep=current_timestep)
    else:
        p = do_nothing_s
        r = do_nothing_s
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        p_a, r_a = select_sito_method(x, grid_sizes)
